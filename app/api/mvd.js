...
const fetchAPI = async (url: string, options: Object = {}) => {
  ...
  console.log(`Fetching url: ${`${BASE_URL}/${url}`} with options ${JSON.stringify(opts)}`)
...
export const getStopVariants = async (stopId: number): Promise<StopVariants> => {
  ...
  const data: StopVariantsAPI = await fetchAPI(`transporteRest/variantes/${stopId}`)
...
export const getStopNextETAs = async (
  ...
  console.log(`Asking next ETAs for stop ${stopId} with variants ${JSON.stringify(variantsCodes)}`)
